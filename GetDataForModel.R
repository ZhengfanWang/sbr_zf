...
# Georgia from 2000 to 2008, definition ge22wks, has administrative data(UN IGME 2019) and GEOSTAT data 
# duplicate data
SBR.full %>% filter(country=="Georgia" &
                    definition == "ge22wks" &
                    year>=2000 &
                    context == "Vital Registration")

### india 2011.5 subnat.LR,  missing NMR 9 observations from 9 different facilityies.
SBR.full %>% filter(country=="India" &
                      definition == "ge28wks" &
                      year == 2011.5)
dim(SBR.full)
...
ge500g_low <- cache[cache$definition=="ge500g"&cache$shmdg2!=1,]
ge1000g_low <- cache[cache$definition=="ge1000g"&cache$shmdg2!=1,]
...
dim(SBR.full)
length(which(is.na(SBR.full$exclusion_notes)*is.na(SBR.full$exclusion_ratio)==1))   


SBR.full[SBR.full$country=="Andorra",] %>% filter(definition=="ge28wks")


SBR.full$int.year <- round(SBR.full$year)

### exclude according to exclusion notes
...
SBR.cache <- SBR.full.exclude %>% select("iso","int.year","definition","source","SBR","adj_sbr_unknown","country","SE","rSN_UN","exclusion_notes","exclusion_ratio") #6518
...
#saveRDS(SBR.plot,file = "excludeRatio.rds")
#saveRDS(SBR.plot,file = "excludeRatio.rds")
...
model_data <- SBR.plot %>% select(iso,int.year,definition,source,SBR,adj_sbr_unknown,country,SE)
