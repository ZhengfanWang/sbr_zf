...
# Note: exclusions applied in reverse order from above b/c most recent overwrites exclusion_notes
...
admin.full <- admin.ori %>% dplyr::rename("country"="Country",
  ...
  mutate(exclusion_notes = replace(exclusion_notes,country=="Cuba" & year==2003 & definition=="x500g" & source_name=="National Statistical Office",
                                   "duplicate observation")) %>%
  mutate(exclusion_notes = replace(exclusion_notes, country=="Cyprus" & notes == "national public sector only",
...
#table(admin.full$exclusion_notes)
