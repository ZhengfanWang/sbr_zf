data{
 ...
 //does not work 
  ...
  int<lower=0> D;                 // 2nd order p-splines
  int<lower=0> H;
  matrix[yearLength,D] BG_td;        // 
...
//parameters: P spline 2 order
 ...
 matrix[numregion,D] gamma_r;
 matrix[numcountry,D] gamma_c;
...
for(c in 1:numcountry){
    for(t in 1:yearLength){
  delta_ct[c,t] = dot_product(BG_td[t,],gamma_c[c,]) +
...
for(i in 1:D){
gamma_r[,i] ~ normal(0,1);
for(c in 1:numcountry){
gamma_c[c,i] ~ normal(gamma_r[getr_c[c],i],sigma_c);
}}
