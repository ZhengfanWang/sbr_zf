...
\item Proposal:  Calculate the probability of observing something more extreme then the observed ratio $r_i$ under the fitted model for log-ratios from step 1:
		\begin{itemize}
			...
			$$N(\hat{\mu}, \hat{\delta}^2 +  \hat{\sigma}^2 + v_i), $$ where $\hat{\mu}$ is the point estimate for $\mu$ and $\hat{\delta}^2$ its posterior variance, and $\hat{\sigma}^2$ is the point estimate for the variance of the random effects.       \item Decision rule: if $p_i< 0.05$, exclude observation $i$.
\end{itemize}
\end{itemize}
\end{enumerate}
...
```
The cutoff value is about 0.53 assuming $v_i = 0$. About 11.5% observation will be excluded by the approach.
```{r}
