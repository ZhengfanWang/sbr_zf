...
mod_24_hic <- jags.model(file = "model_subset.txt",
                     data = jags_data,
                     n.chains = 5,
                     n.adapt = 5000)
update(mod_24_hic, 5000) # burn-in
samp_24_hic <- coda.samples(mod_24_hic, c("mu","sigma","k_s"), n.iter = 100000, thin=10)
autocorr.plot(samp_24_hic) # all fine
gelman.diag(samp_24, autoburnin = F) # good

summary(samp_24_hic)
plot(samp_24_hic)
summary(samp_24_hic)$quantiles[1,"50%"]
summary(samp_24_hic)$statistics[1,2]

save(samp_24_hic, file="samp_24.Rdata")
