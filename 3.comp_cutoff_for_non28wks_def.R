...
def_adj_res <- data.frame(definition_rv=set_definition_rv,
                          lmic=lmic,
                          def_bias=def_bias,
                          def_sd=def_sd)
def_adj_res
...
defadj_sbr2018 <- defadj_sbr2018 %>% mutate(#defadj_exclude_sbrnmr_max= replace(defadj_exclude_sbrnmr_max,defadj_exclude_sbrnmr_max==TRUE & WPP_LB <= 30000 & (SBR==0 | NMR==0),FALSE),
